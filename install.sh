...
demyx_install() {
    ...
    if [[ "$DEMYX_INSTALL" != *"--no-ping"* ]]; then
        ...
        docker run -t --rm demyx/utilities curl -s "https://demyx.sh/?action=active&version=1.8.1&token=V1VpdGNPcWNDVlZSUDFQdFBaR0Zhdz09OjrnA1h6ZbDFJ2T6MHOwg3p4" -o /dev/null
    fi
