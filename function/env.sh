...
demyx_env() {
    ...
    if [[ -f "$DEMYX_ENV"/.env ]]; then
        ...
        # Auto upgrade php to 8.4 when version is 8.3 or below
        case "${DEMYX_APP_OLS_LSPHP:-}" in
            8.1|8.2|8.3|81|82|83) DEMYX_APP_OLS_LSPHP=8.4 ;;
        esac
        case "${DEMYX_APP_PHP:-}" in
            8.1|8.2|8.3|81|82|83) DEMYX_APP_PHP=8.4 ;;
        esac
    else
    ...
    if [[ -n "$DEMYX_ENV_GREP" ]]; then
        ...
        DEMYX_APP_WP_VOLUME="${DEMYX_APP_WP_CONTAINER}"
    fi
...
DEMYX_APP_OLS_LSPHP=${DEMYX_APP_OLS_LSPHP:-8.4}
...
DEMYX_APP_PHP=${DEMYX_APP_PHP:-8.4}
