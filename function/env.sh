...
demyx_env() {
    ...
    if [[ "$DEMYX_RUN_TYPE" = wp || "$DEMYX_APP_TYPE" = wp ]]; then
        echo "# AUTO GENERATED
            ...
            DEMYX_APP_WP_IMAGE=${DEMYX_APP_WP_IMAGE:-demyx/openlitespeed}
            DEMYX_APP_STACK=${DEMYX_APP_STACK:-ols}
            ...
            DEMYX_APP_PHP_PM_MAX_CHILDREN=${DEMYX_APP_PHP_PM_MAX_CHILDREN:-100}
            DEMYX_APP_PHP_PM_START_SERVERS=${DEMYX_APP_PHP_PM_START_SERVERS:-10}
            DEMYX_APP_PHP_PM_MIN_SPARE_SERVERS=${DEMYX_APP_PHP_PM_MIN_SPARE_SERVERS:-5}
            DEMYX_APP_PHP_PM_MAX_SPARE_SERVERS=${DEMYX_APP_PHP_PM_MAX_SPARE_SERVERS:-25}
            DEMYX_APP_PHP_PM_PROCESS_IDLE_TIMEOUT=${DEMYX_APP_PHP_PM_PROCESS_IDLE_TIMEOUT:-5s}
            ...
            DEMYX_APP_OLS_CLIENT_THROTTLE_STATIC=${DEMYX_APP_OLS_CLIENT_THROTTLE_STATIC:-50}
            DEMYX_APP_OLS_CLIENT_THROTTLE_DYNAMIC=${DEMYX_APP_OLS_CLIENT_THROTTLE_DYNAMIC:-50}
            ...
            DEMYX_APP_OLS_CLIENT_THROTTLE_SOFT_LIMIT=${DEMYX_APP_OLS_CLIENT_THROTTLE_SOFT_LIMIT:-250}
            DEMYX_APP_OLS_CLIENT_THROTTLE_HARD_LIMIT=${DEMYX_APP_OLS_CLIENT_THROTTLE_HARD_LIMIT:-500}
            DEMYX_APP_OLS_CLIENT_THROTTLE_BLOCK_BAD_REQUEST=${DEMYX_APP_OLS_CLIENT_THROTTLE_BLOCK_BAD_REQUEST:-1}
            DEMYX_APP_OLS_CLIENT_THROTTLE_GRACE_PERIOD=${DEMYX_APP_OLS_CLIENT_THROTTLE_GRACE_PERIOD:-15}
            DEMYX_APP_OLS_CLIENT_THROTTLE_BAN_PERIOD=${DEMYX_APP_OLS_CLIENT_THROTTLE_BAN_PERIOD:-60}
            DEMYX_APP_OLS_TUNING_MAX_CONNECTIONS=${DEMYX_APP_OLS_TUNING_MAX_CONNECTIONS:-1000}
            DEMYX_APP_OLS_TUNING_CONNECTION_TIMEOUT=${DEMYX_APP_OLS_TUNING_CONNECTION_TIMEOUT:-60}
            DEMYX_APP_OLS_TUNING_MAX_KEEP_ALIVE=${DEMYX_APP_OLS_TUNING_MAX_KEEP_ALIVE:-500}
            DEMYX_APP_OLS_TUNING_SMART_KEEP_ALIVE=${DEMYX_APP_OLS_TUNING_SMART_KEEP_ALIVE:-0}
            DEMYX_APP_OLS_TUNING_KEEP_ALIVE_TIMEOUT=${DEMYX_APP_OLS_TUNING_KEEP_ALIVE_TIMEOUT:-5}
            DEMYX_APP_OLS_PHP_LSAPI_CHILDREN=${DEMYX_APP_OLS_PHP_LSAPI_CHILDREN:-10}
            ...
            DEMYX_APP_OLS_RECAPTCHA_CONNECTION_LIMIT=${DEMYX_APP_OLS_RECAPTCHA_CONNECTION_LIMIT:-100}
            ...
            MARIADB_MAX_CONNECTIONS=${MARIADB_MAX_CONNECTIONS:-100}
        " | sed 's|            ||g' > "$DEMYX_WP"/"$DEMYX_TARGET"/.env
    fi
}
demyx_stack_env() {
    ...
    echo "# AUTO GENERATED
        DEMYX_STACK_SERVER_IP=${DEMYX_STACK_SERVER_IP:-$(curl -s https://ipecho.net/plain)}
        DEMYX_STACK_SERVER_API=${DEMYX_STACK_SERVER_API:-false}
        DEMYX_STACK_AUTH=$DEMYX_PARSE_BASIC_AUTH
        DEMYX_STACK_API=${DEMYX_STACK_API:-false}
        DEMYX_STACK_TELEMETRY=${DEMYX_STACK_TELEMETRY:-true}
        DEMYX_STACK_DOMAIN=$DEMYX_STACK_DOMAIN
        DEMYX_STACK_AUTO_UPDATE=${DEMYX_STACK_AUTO_UPDATE:-true}
        DEMYX_STACK_MONITOR=${DEMYX_STACK_MONITOR:-true}
        DEMYX_STACK_HEALTHCHECK=${DEMYX_STACK_HEALTHCHECK:-true}
        DEMYX_STACK_HEALTHCHECK_TIMEOUT=${DEMYX_STACK_HEALTHCHECK_TIMEOUT:-5}
        DEMYX_STACK_BACKUP=${DEMYX_STACK_BACKUP:-true}
        DEMYX_STACK_BACKUP_LIMIT=${DEMYX_STACK_BACKUP_LIMIT:-30}
        DEMYX_STACK_CPU=${DEMYX_STACK_CPU:-$DEMYX_CPU}
        DEMYX_STACK_MEM=${DEMYX_STACK_MEM:-$DEMYX_MEM}
        DEMYX_STACK_OUROBOROS=${DEMYX_STACK_OUROBOROS:-true}
        DEMYX_STACK_OUROBOROS_IGNORE=\"${DEMYX_STACK_OUROBOROS_IGNORE:-demyx_socket}\"
        DEMYX_STACK_ACME_EMAIL=$DEMYX_STACK_ACME_EMAIL
        DEMYX_STACK_ACME_STORAGE=${DEMYX_STACK_ACME_STORAGE:-/demyx/acme.json}
        DEMYX_STACK_CLOUDFLARE=${DEMYX_STACK_CLOUDFLARE:-false}
        DEMYX_STACK_CLOUDFLARE_EMAIL=$DEMYX_STACK_CLOUDFLARE_EMAIL
        DEMYX_STACK_CLOUDFLARE_KEY=$DEMYX_STACK_CLOUDFLARE_KEY
        DEMYX_STACK_LOG_LEVEL=${DEMYX_STACK_LOG_LEVEL:-INFO}
        DEMYX_STACK_TRUSTED_IPS=173.245.48.0/20,103.21.244.0/22,103.22.200.0/22,103.31.4.0/22,141.101.64.0/18,108.162.192.0/18,190.93.240.0/20,188.114.96.0/20,197.234.240.0/22,198.41.128.0/17,162.158.0.0/15,104.16.0.0/12,172.64.0.0/13,131.0.72.0/22
    " | sed 's|        ||g' > "$DEMYX_STACK"/.env
}
