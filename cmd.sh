...
if [ "$1" = "stack" ]; then
    ...
    if [ "$ACTION" = up ] && [ "$SERVICE" ]; then
        docker-compose up -d $SERVICE
    elif [ "$ACTION" = up ] && [ ! "$SERVICE" ]; then
        docker-compose up -d
    elif [ "$ACTION" = down ] && [ "$SERVICE" ]; then
    ...
    if [ "$ACTION" ]; then
        ...
        [[ ! "$ALL" ]] && [[ "$DOMAIN" ]] && cd $CONTAINER_PATH && source .env

        if [ "$ACTION" = up ] && [ ! -z "$SERVICE" ] && [ "$DOMAIN" ]; then
            docker-compose up -d ${CONTAINER_NAME}_$SERVICE
        elif [ "$ACTION" = up ] && [ ! "$ALL" ] && [ "$DOMAIN" ]; then
            docker-compose up -d
        elif [ "$ACTION" = up ] && [ "$ALL" ]; then
            for i in $LIST
            do
                cd $APPS/$i
                [[ -f $APPS/$i/data/wp-config.php ]] && cd $APPS/$i && docker-compose up -d || echo -e "\e[33m[WARNING] Skipping $i\e[39m"
            done
        elif [ "$ACTION" = down ] && [ ! "$ALL" ] && [ "$DOMAIN" ]; then
            docker-compose stop && docker-compose rm -f
        elif [ "$ACTION" = down ] && [ "$ALL" ]; then
            for i in $LIST
            do
                cd $APPS/$i
                [[ -f $APPS/$i/data/wp-config.php ]] && cd $APPS/$i && docker-compose stop && docker-compose rm -f || echo -e "\e[33m[WARNING] Skipping $i\e[39m"
            done
        elif [ "$ACTION" ] && [ ! "$SERVICE" ] && [ "$DOMAIN" ]; then
            docker-compose $ACTION
        elif [ "$ACTION" ] && [ "$SERVICE" ] && [ "$DOMAIN" ]; then
            [[ "$SERVICE" = "wp" ]] && docker-compose $ACTION wp_${WP_ID} || docker-compose $ACTION db_${WP_ID}
        else
            echo
        ...
        cd $CONTAINER_PATH
        source .env
        [[ "$SERVICE" = db ]] && docker-compose exec db_${WP_ID} $CLI || docker-compose exec wp_${WP_ID} $CLI
    elif [ "$CLONE" ]; then
        ...
        if [ "$PMA" = "on" ]; then
            [[ "$PMA_EXIST" ]] && docker stop phpmyadmin && docker rm phpmyadmin
        ...
        if [ "$ALL" ]; then
            ...
            [[ ! "$DOMAIN" ]] && die 'Domain is missing or add --all'
        ...
        if [ "$ALL" ]; then
            ...
            [[ ! "$DOMAIN" ]] && die 'Domain is missing, use --dom or --restart=domain.tld'
        ...
        echo -e "\e[33m"
        [[ ! "$DOMAIN" ]] && read -ep "[WARNING] Delete all sites? [yY]: " DELETE_SITE || read -ep "[WARNING] Delete/overwrite $DOMAIN? [yY]: " DELETE_SITE
        echo -e "\e[39m"
        ...
        if [ "$ALL" ]; then
            ...
            [[ -f $LOGS/$DOMAIN* ]] && rm $LOGS/$DOMAIN*.log
        ...
        echo "Password: $WORDPRESS_USER_PASSWORD"
        echo
    elif [ "$DEMYX_SHELL" ]; then
        source $CONTAINER_PATH/.env
        [[ "$DEMYX_SHELL" = "wp" ]] && docker exec -it $WP bash || docker exec -it $DB sh
    elif [ "$SCALE" ]; then
        cd $CONTAINER_PATH
        source .env
        [[ ! "$SERVICE" ]] && die '--service is missing or empty'
        [[ "$SERVICE" = wp ]] && docker-compose up -d --scale wp_${WP_ID}=$SCALE wp_${WP_ID} || docker-compose up -d --scale db_${WP_ID}=$SCALE db_${WP_ID}
    elif [ "$SSL" ]; then
        bash $ETC/functions/yml.sh $CONTAINER_PATH $SSL
        demyx wp --dom=$DOMAIN --up
    elif [ "$UPDATE" ]; then
    ...
    [[ ! "$1" ]] && echo && echo -e "\e[34m[INFO] See commands for help: demyx -h, demyx stack -h, demyx wp -h\e[39m" && echo
    while :; do
        case $1 in
            -h|-\?|--help)
                echo 
            ...
            -u|--update)
                ...
                [[ "$FORCE" ]] && echo -e "\e[33m[WARNING] Forcing an update for Demyx...\e[39m" || echo -e "\e[34m[INFO] Checking for updates\e[39m"
                CHECK_FOR_UPDATES=$(git pull | grep "Already up to date." )

                if [ "$FORCE" ] || [ "$CHECK_FOR_UPDATES" != "Already up to date." ]; then
                    [[ ! "$FORCE" ]] && echo -e "\e[34m[INFO] Updating Demyx...\e[39m"
