...
RUN set -ex; \
	...
	apk add --virtual .elgg-phpexts-rundeps $runDeps; \
	apk del .build-deps
...
RUN set -ex; \
	mkdir -p /usr/src; \
	curl -o elgg.zip -fSL "https://elgg.org/about/getelgg?forward=elgg-${ELGG_VERSION}.zip"; \
	unzip elgg.zip -d /usr/src/; \
	rm elgg.zip; \
	mv /usr/src/elgg-$ELGG_VERSION /usr/src/elgg; \
